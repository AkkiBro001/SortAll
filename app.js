...
const colors = [{color:'red', count:COLORS_COUNT}, {color:'green', count:COLORS_COUNT}, {color:'blue', count:COLORS_COUNT}, {color:'cyan', count:COLORS_COUNT}, {color:'orange', count:COLORS_COUNT}];
const CREATE_EMPTY_TUBE = (noOfEmptyTube) => {let arr = []; for(i=0;i<noOfEmptyTube;i++){arr.push([""])} return arr}
const container = [...colors.map((_)=>[]), ...CREATE_EMPTY_TUBE(NUM_EMPTY_TUBE)];
...
function loadInitialTubs(){
    while(colors.length > 0){
        for(let tube=0; tube < container.length-NUM_EMPTY_TUBE; tube++){
            ...
            if(container[tube].length < COLORS_COUNT){
...
function handleTube(e) {
    ...
    if(!isActive){
        ...
        if(
            container[tube.dataset.id].length === 4 || 
        ...
        if(container[tube.dataset.id][0] == container[firstTubeIndex][0] || !container[tube.dataset.id][0] || !container[firstTubeIndex][0] ){
            function dropColor(){
            ...
            if(color === container[firstTubeIndex][0] && container[tube.dataset.id].length < 4){
                dropColor()
            }
...
function checkColors(){
    ...
    const isSort = container.every((tube) => tube.every((colors,_,arr) => {
        if(arr[0] === colors){
